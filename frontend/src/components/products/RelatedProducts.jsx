...
const RelatedProducts = ({ productId, product }) => {
  const {
    ...
    scrollPrev,
    scrollNext,
  } = useRelatedProducts(productId, product);
  ...
  if (isError) {
    return (
      <ErrorState
        ...
        description="We're unable to load related products right now. Please try refreshing the page or check back later."
  ...
  if (!hasProducts) {
    return (
      <ErrorState
        title="No products available"
        description="There are no related products to display at this time. Please check back soon!"
        minHeight="min-h-[200px]"
      />
    );
  }
